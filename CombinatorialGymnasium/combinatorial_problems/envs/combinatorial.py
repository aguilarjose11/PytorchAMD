...
class Phase1Env(gym.Env):
    ...
    def _step_model(self, prev_node, next_node, action):
        ...
        if terminated or self.iter == 49:  # 49  is maximum number of iterations in environment
        ...
        return distance, vis_obj_reward, non_vis_obj_penalty, terminated
    ...
    def step(self,
        ...
        distance, vis_obj_reward, non_vis_obj_penalty, terminated = self._step_model(prev_node, new_node, action)
        ...
        reward = (-1) * distance + self.time_penalty + vis_obj_reward + non_vis_obj_penalty
...
class Phase3Env(gym.Env):
    ...
    def __init__(self,
                 ...
                 time_penalty: float = -1.0,
                 visited_reward: float = 50.0,
                 max_velocity: float = 0.075
                 ):
        """Phase 3 Problem
        Find shortest path between a set of objective nodes given a graph. The agent will start at a random location,
        where two sets of nodes will be returned (index 0 for list of objective nodes, and index 1 for non-objective
        nodes). The graph is assumed to be fully connected. Phase 3 differs from Phase 1 in that some nodes
        now are obstacles with negative reward radius (1/x), along with dynamic obstacles. Agent also contains
        greedy baseline that selects the nearest objective at each timestep. In addition, Phase 3 differs from Phase 2
        in that the obstacles are allowed to change locations by the maximum velocity.
        ...
        visited_reward: float
            The reward for visiting objective nodes (only the first time).
        max_velocity: float
            Maximum velocity for each obstacle.
        """
        self.num_obstacles = num_obstacles
        self.random_obstacles = random_obstacles
        self.obstacle_max_distance = obstacle_max_distance
    ...
    def reset(self,
        ...
        if self.new_on_reset or not hasattr(self, "nodes"):
            ...
            objectives = np.zeros(self.num_nodes)  # boolean encoding
            obstacles = np.zeros(self.num_nodes)  # boolean encoding
            velocities = np.zeros((self.num_nodes, 2))   # initial x, y velocity for obstacles
            ...
            for i in idx:  # calculate velocities for obstacles
                velo = self.np_random.uniform(-self.max_velocity, self.max_velocity,  2)
                while np.any(nodes[i] + velo > 1) or np.any(nodes[i] + velo < 0):
                    velo = self.np_random.uniform(-self.max_velocity, self.max_velocity, 2)
                velocities[i] = velo

            self.nodes = np.hstack((nodes, objectives.reshape(-1, 1), end_node.reshape(-1, 1),
                                    obstacles.reshape(-1, 1), velocities))
    ...
    def _update_turrets_non_objectives(self):
        ...
        for i in self.obstacle_idx:  # update velocities for obstacles
            velo = self.np_random.uniform(-self.max_velocity, self.max_velocity, 2)
